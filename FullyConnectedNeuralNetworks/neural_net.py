...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # ================================================================ #
    f= lambda x: x*(x>0)
    H1=f(np.dot(W1,X.T)+b1[:,np.newaxis])
    scores=(np.dot(W2,H1)+b2[:,np.newaxis]).T
    ...
    #loss=(1/X.shape[0])*(np.log(np.exp(scores).sum(axis=1))-scores[ range(len(scores)),y]).sum()
    #prob = np.exp(scores-scores[index])/np.sum(np.exp(scores-scores[index]), axis=1)[:,np.newaxis]
    scores=scores-scores[index]
    #prob = np.exp(scores-scores[index])/np.sum(np.exp(scores-scores[index]), axis=1,keepdims=True)
    prob = np.exp(scores)/np.sum(np.exp(scores), axis=1,keepdims=True)
    loss=-(1/len(y))*np.log((prob[np.arange(len(y)),y])).sum()+reg*0.5*(norm(W1)+norm(W2))
   ...
   # print(prob.shape)
    ...
    temp=np.copy(prob)
    temp[np.arange(len(y)),y]=temp[np.arange(len(y)),y]-1
    temp=temp/len(y)

    grads['W2']=np.dot(H1,temp).T+W2*reg
    grads['b2']=temp.sum(axis=0)
    rslt=np.dot(temp,W2)
    temp2=np.dot(X,W1.T)+b1
    temp2=(temp2>0)
    result=rslt*temp2
    grads['W1']=np.dot(X.T,result).T+W1*reg
    grads['b1']=result.sum(axis=0)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in np.arange(num_iters):
      ...
      # YOUR CODE HERE:
  	  ...
  	  # ================================================================ #
  	  #pass
      index_batch=np.random.choice(num_train,batch_size)
  ...
  def predict(self, X):
    ...
    # ================================================================ #
    f= lambda x: x*(x>0)
    H1=f(np.dot(self.params['W1'],X.T)+self.params['b1'][:,np.newaxis])
    scores=(np.dot(self.params['W2'],H1)+self.params['b2'][:,np.newaxis]).T
