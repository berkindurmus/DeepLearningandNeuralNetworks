...
class SVM(object):
  ...
  def loss(self, X, y):
    ...
    for i in np.arange(num_train):
        for j in range (num_classes):
            if j!=y[i] :
                loss=loss+np.maximum(0,1+a[i,j]-a[i,y[i]])
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    loss = 0.0
    y_inverse_onehot=np.abs(y_onehot-1)
    
    grad = np.zeros(self.W.shape) # initialize the gradient as zero
    a=np.dot(X,self.W.T)
    b=(np.multiply(a,y_onehot)).sum(axis=1)
    c=(a-b[:,np.newaxis])+1
    d=c.clip(min=0)
    
    f=np.multiply(d,y_inverse_onehot)
    loss=f.sum().sum()
    loss=(1/X.shape[0])*loss
    ...
    d[range(X.shape[0]),y]-= np.sum(d,axis=1)
    grad = d.T.dot(X)
    grad /= X.shape[0]
	# ================================================================ #
    # YOUR CODE HERE:
